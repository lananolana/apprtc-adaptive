...
|---|---|---|---|---|
| A | Один ноутбук Apple MacBook (macOS 14+), две вкладки Chrome | Один сетевой интерфейс OS | Network Link Conditioner (системно) | На одном устройстве |
| B | Два устройства (ноутбук + телефон или второй ноутбук) | Общая домашняя сеть Wi-Fi | Реальное выключение Wi-Fi на одной стороне | На основном устройстве (опционально — на обоих) |
...
- **Время реакции.** Для сегмента под нагрузкой определяется временной
  ...
  применения политики).
- **Время восстановления.** Для прогонов уровня B измеряется временной
  интервал от восстановления физического Wi-Fi-подключения до
  возобновления видеопотока (`bytesReceived` снова растёт).
...
- **Принятые решения политики.** Для режима *adaptive* подсчитывается
  ...
  перехода в режим audio-only и срабатывания RecoveryManager.

Сравнение двух режимов на уровне A проводится попарно по идентичным
сетевым профилям; метрики сопоставляются по сегментам «load» и «post»
для оценки эффекта во время и после периода деградации. На уровне B
сопоставляется поведение прототипа в двух режимах в идентичном
сценарии обрыва. Уровень C интерпретируется как case study и не
является основой количественных выводов.
...
Таблица 3.3 — Сводные показатели для профиля `good` (плейсхолдер)
...
Таблица 3.4 — Сравнение режимов при профиле `3g` (плейсхолдер)
...
Таблица 3.5 — Сравнение режимов при профиле `edge` (плейсхолдер)
...
Таблица 3.6 — Сравнение режимов при профиле `lossy` (плейсхолдер)
...
Таблица 3.7 — Сравнение режимов при профиле `severe` (плейсхолдер)
...
приведено в таблице 3.10 и иллюстрируется графиком динамики
видеобитрейта (рисунок 3.3) и динамики QoE-оценки (рисунок 3.4) на
протяжении всего эксперимента.

Таблица 3.10 — Обобщённое сравнение режимов *baseline* и *adaptive* (плейсхолдер)
