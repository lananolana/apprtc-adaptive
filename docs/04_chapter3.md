...
`apply(decision)`. Все параметры кодирования — разрешение, частота
кадров и битрейт — устанавливаются единственным вызовом
`RTCRtpSender.setParameters()` через поля `encodings[0]`:
`scaleResolutionDownBy` задаёт коэффициент даунскейла относительно
реального разрешения захвата (вычисляется из `track.getSettings()`),
`maxFramerate` ограничивает частоту кадров, `maxBitrate` — пиковый
битрейт. Метод `MediaStreamTrack.applyConstraints()` сознательно не
