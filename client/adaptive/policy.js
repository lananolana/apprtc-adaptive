...
export const LADDER = [
  ...
  //
  // В версии v4 пересмотрена связка fps↔ступень. Раньше fps падал вместе с
  // разрешением (10/15/20/24), что отбирало у пользователя плавность даже
  // на ступенях, где она ещё технически достижима. Эксперимент на edge
  // показал, что встроенный congestion control браузера в тех же условиях
  // спокойно держит 25-30 fps на 360p, лишь снижая битрейт. Поэтому в v4
  // на 360p и выше задан fps=30; 240p оставлен на 20 (компромисс с
  // битрейтом); только нижняя 180p-ступень допускает 15 fps как «спасение
  // картинки» при минимальной полосе.
  { name: '180p@15',  width: 320,  height: 180,  fps: 15, maxBitrateKbps:  200 },
  { name: '240p@20',  width: 426,  height: 240,  fps: 20, maxBitrateKbps:  350 },
  { name: '360p@30',  width: 640,  height: 360,  fps: 30, maxBitrateKbps:  700 },
  { name: '480p@30',  width: 854,  height: 480,  fps: 30, maxBitrateKbps: 1400 },
...
const DEFAULTS = {
  ...
  lossBadPct:    7,
  lossGoodPct:   1,
  // Условие audio-only. Версия v4: дополнительно смягчены пороги после
  // экспериментов на severe-профиле, где AND-условие v3 (rtt>2000 &&
  // loss>15) при пиковых значениях rtt~2.5 с и loss~25 % уводило adaptive
  // в audio-only на ~50 % времени, тогда как встроенный congestion control
  // в тех же условиях продолжал передавать видео на 360p. Подняли порог
  // loss с 15 до 25 % и extremeStreakN с 3 до 4 — теперь audio-only
  // активируется только при действительно катастрофическом канале
  // (≥4 с подряд rtt>2 с И loss>25 %), а в зоне «тяжело, но возможно»
  // мы стараемся вытащить видео битрейтом и разрешением.
  audioOnlyRttMs:    2000,
  audioOnlyLossPct:  25,
  extremeStreakN:    4,
};
